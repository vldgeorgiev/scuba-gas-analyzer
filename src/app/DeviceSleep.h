...
inline bool enterDeviceSleep(DisplayManager& display) {
  const auto wakePin = static_cast<gpio_num_t>(PIN_BUTTON_2);
  if (digitalRead(PIN_BUTTON_2) == LOW || digitalRead(PIN_BUTTON_1) == LOW || display.touchActive()) return false;
  if (esp_sleep_pd_config(ESP_PD_DOMAIN_RTC_PERIPH, ESP_PD_OPTION_ON) != ESP_OK ||
      ...
      esp_sleep_enable_ext1_wakeup(1ULL << PIN_BUTTON_2, ESP_EXT1_WAKEUP_ANY_LOW) != ESP_OK) {
  ...
  if (ready && previousWifi != WIFI_OFF) ready = WiFi.mode(WIFI_OFF);
