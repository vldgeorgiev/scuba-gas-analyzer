...
static void event_handler_cb_log_obj6(lv_event_t *e) {
    lv_event_code_t event = lv_event_get_code(e);
    void *flowState = lv_event_get_user_data(e);
    if (event == LV_EVENT_VALUE_CHANGED) {
        lv_obj_t *ta = lv_event_get_target(e);
...
void tick_screen_config() {
    ...
    }
    {
        ...
        const char *cur_val = lv_dropdown_get_options(objects.obj4);
        if (strcmp(new_val, cur_val) != 0) {
            tick_value_change_obj = objects.obj4;
            lv_dropdown_set_options(objects.obj4, new_val);
            tick_value_change_obj = NULL;
        }
    }
    {
        if (!(lv_obj_get_state(objects.obj4) & LV_STATE_EDITED)) {
            ...
            int32_t cur_val = lv_dropdown_get_selected(objects.obj4);
            if (new_val != cur_val) {
                tick_value_change_obj = objects.obj4;
                lv_dropdown_set_selected(objects.obj4, new_val);
                tick_value_change_obj = NULL;
            }
        }
    }
    {
        ...
        const char *cur_val = lv_dropdown_get_options(objects.obj5);
        if (strcmp(new_val, cur_val) != 0) {
            tick_value_change_obj = objects.obj5;
            lv_dropdown_set_options(objects.obj5, new_val);
            tick_value_change_obj = NULL;
        }
    }
    {
        if (!(lv_obj_get_state(objects.obj5) & LV_STATE_EDITED)) {
            int32_t new_val = evalIntegerProperty(flowState, 12, 4, "Failed to evaluate Selected in Dropdown widget");
            int32_t cur_val = lv_dropdown_get_selected(objects.obj5);
            if (new_val != cur_val) {
                tick_value_change_obj = objects.obj5;
                lv_dropdown_set_selected(objects.obj5, new_val);
                tick_value_change_obj = NULL;
            }
        }
    }
...
void create_screen_log() {
    ...
    lv_obj_set_size(obj, 320, 170);
    {
        lv_obj_t *parent_obj = obj;
        {
            ...
            lv_obj_add_event_cb(obj, event_handler_cb_log_obj6, LV_EVENT_ALL, flowState);
        }
        {
            // btnHome_1
            lv_obj_t *obj = lv_btn_create(parent_obj);
